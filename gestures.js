...
(() => {
 ...
 const allowed=()=>U.view==='live'&&!S.recMode&&!U.draw&&!U.sheet&&!U.menu&&!U.picker&&!REC;
 document.addEventListener('pointerdown',e=>{
  if(e.pointerType!=='touch')return;
  touches.add(e.pointerId);
  if(touches.size>1){start=null;org=null;clearHold();clearHl();return;}
  if(!allowed()||e.clientX<24||e.clientX>innerWidth-24||!e.target.closest('#app>.scroll')||e.target.closest('button,input,textarea,select,.pull'))return;
...
<li><b>自分の指摘を見る</b><br>下の名前欄で自分を選び、「指摘のみ」を押します。「次の指摘」で順番に確認でき、「全文に戻す」で歌詞全体へ戻れます。指摘のみの表示は通常の歌詞画面で使えます。</li>
