...
function sheet() {
 ...
 const c=vm.createContext({U:{sheet:{lineIdx:0,range:[1,3],tags:[],sel:['member'],memo:'',seq:['C4']}},S:{notes:[],showId:'show'},overlay:null,
  ...
  document:{addEventListener(name,fn){click=fn;},getElementById:id=>id==='memo'&&c.U.sheet.detail?{value:memo}:null,
   createElement(){return {innerHTML:'',remove(){},querySelector(selector){if(selector==='.quick-note')return {offsetWidth:312,offsetHeight:372,style:{}};if(selector==='.quick-dial')return {offsetTop:44,offsetHeight:264};return this.innerHTML.includes('note-details-content')?{open:true,addEventListener(){},scrollIntoView(){}}:null;}};},body:{appendChild(el){html=el.innerHTML;}}},
...
test('all 34 choices are directly available in the circular palette, separate from scrolling tools',()=>{
...
test('quick menu opens with the selected text and preserves state across all palettes and tools',()=>{
 ...
 const html=s.render();
 assert(html.includes('aria-label="8方向で指摘"'));assert(html.includes('しい歌'));
 assert.equal((html.match(/class="quick-direction /g)||[]).length,8);
